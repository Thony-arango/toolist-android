...
fun AgregarProductoScreen(
    ...
    // Sync selectedList with initialListId when lists load
    LaunchedEffect(uiState.lists) {
        if (selectedList == null && uiState.lists.isNotEmpty()) {
            selectedList = uiState.lists.firstOrNull()?.id
        }
    }

    val selectedListName = uiState.lists.firstOrNull { it.id == selectedList }?.name ?: ""
        ...
        Column(
            ...
            Button(
                ...
                if (uiState.isLoading) {
                    ...
                    Text(
                        text = stringResource(R.string.add_product_btn_add),
                        style = MaterialTheme.typography.labelLarge,
                    )
                }
            }
...
@Composable
private fun AgregarProductoScreenPreview() {
    ToolistTheme {
